...
def kd0_loss(logits_student_in, logits_teacher_in, temperature, reduce=True, logit_stand=False):
...
def kd_loss(logits_student_in, logits_teacher_in, temperature, alpha=0.5, beta=0.5, reduce=True, logit_stand=False):
    logits_student = normalize(logits_student_in) if logit_stand else logits_student_in
    logits_teacher = normalize(logits_teacher_in) if logit_stand else logits_teacher_in
    ...
    if reduce:
        loss_akd = (
            ((alpha * loss_part1 + (1 - alpha) * loss_part2)).sum(0)
            / logits_student.shape[0]
        )
    else:
        loss_akd = ((loss_part1 + loss_part2)).sum(0)
