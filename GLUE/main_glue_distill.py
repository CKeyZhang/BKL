...
def parse_args():
    ...
    parser.add_argument("--kl_type", type=str, default="fkl",
                        help="We have:fkl,rkl,fkl+rkl,akl,bkl,dkl")
...
initial_memory = torch.cuda.memory_allocated(args.device)
...
if args.do_train:
    ...
    final_memory = torch.cuda.memory_allocated(args.device)
    ...
    results_file = os.path.join(args.checkpoint_dir, "training_results.txt")
    with open(results_file, "w") as f:
        f.write(f"Experiment Name: {args.exp_name}\n")
        f.write(f"Total Training Time: {total_time:.2f} seconds\n")
        f.write(f"Initial GPU Memory: {initial_memory / (1024 ** 3):.2f} GB\n")
        f.write(f"Final GPU Memory: {final_memory / (1024 ** 3):.2f} GB\n")
        f.write(f"Total GPU Memory Usage: {total_memory / (1024 ** 3):.2f} GB\n")
        f.write(f"Best Evaluation Metric ({metric_name}): {eval_best}\n")
